...
from sybil.model.audio import AudioLDMPipeline
from sybil.model.image import StableDiffusionPipeline
from sybil.model.video import TextToVideoSDPipeline

from sybil.utils import l2_loss, process_batch_stage_1, process_batch_stage_2, process_batch_stage_3 
...
class Engine(nn.Module):
    ...
    def _add_image_token(self):
        ...
        self.llm_tokenizer.add_tokens({"<Img>"})  # add special image token to tokenizer
        self.llm_tokenizer.add_tokens({"</Img>"})  # add special image token to tokenizer

        # Add [IMG] tokens to the vocabulary.
        self.config['gen_img_token_idx'] = []
        for i in range(self.config['num_gen_img_tokens']):
            print(f'Adding [IMG{i}] token to vocabulary.')
            print(f'Before adding new token, tokenizer("[IMG{i}]") =',
                  self.llm_tokenizer(f'[IMG{i}]', add_special_tokens=False))
            num_added_tokens = self.llm_tokenizer.add_tokens(f'[IMG{i}]')
            print(f'After adding {num_added_tokens} new tokens, tokenizer("[IMG{i}]") =',
                  self.llm_tokenizer(f'[IMG{i}]', add_special_tokens=False))
            gen_token_idx = self.llm_tokenizer(f'[IMG{i}]', add_special_tokens=False).input_ids
            assert len(gen_token_idx) == 1, gen_token_idx
            self.config['gen_img_token_idx'].append(gen_token_idx[0])

    def _add_video_token(self):
        # self.llm_tokenizer.add_tokens({"<Vid>"})  # add special video token to tokenizer
        # self.llm_tokenizer.add_tokens({"</Vid>"})  # add special video token to tokenizer

        # Add [VID] tokens to the vocabulary.
        self.config['gen_video_token_idx'] = []
        for i in range(self.config['num_gen_video_tokens']):
            print(f'Adding [VID{i}] token to vocabulary.')
            print(f'Before adding new token, tokenizer("[VID{i}]") =',
                  self.llm_tokenizer(f'[VID{i}]', add_special_tokens=False))
            num_added_tokens = self.llm_tokenizer.add_tokens(f'[VID{i}]')
            print(f'After adding {num_added_tokens} new tokens, tokenizer("[VID{i}]") =',
                  self.llm_tokenizer(f'[VID{i}]', add_special_tokens=False))
            gen_token_idx = self.llm_tokenizer(f'[VID{i}]', add_special_tokens=False).input_ids
            assert len(gen_token_idx) == 1, gen_token_idx
            self.config['gen_video_token_idx'].append(gen_token_idx[0])

    def _add_audio_token(self):
        # self.llm_tokenizer.add_tokens({"<Aud>"})  # add special audio token to tokenizer
        # self.llm_tokenizer.add_tokens({"</Aud>"})  # add special audio token to tokenizer

        # Add [AUD] tokens to the vocabulary.
        self.config['gen_audio_token_idx'] = []
        for i in range(self.config['num_gen_audio_tokens']):
            print(f'Adding [AUD{i}] token to vocabulary.')
            print(f'Before adding new token, tokenizer("[AUD{i}]") =',
                  self.llm_tokenizer(f'[AUD{i}]', add_special_tokens=False))
            num_added_tokens = self.llm_tokenizer.add_tokens(f'[AUD{i}]')
            print(f'After adding {num_added_tokens} new tokens, tokenizer("[AUD{i}]") =',
                  self.llm_tokenizer(f'[AUD{i}]', add_special_tokens=False))
            gen_token_idx = self.llm_tokenizer(f'[AUD{i}]', add_special_tokens=False).input_ids
            assert len(gen_token_idx) == 1, gen_token_idx
            self.config['gen_audio_token_idx'].append(gen_token_idx[0])
    ...
    def encode_video(self, video_paths):
        ...
        llm_inputs = self.llm_projection(video_embeds).unsqueeze(1)  # bsz x 1 x llama_size
        atts_llm = torch.ones(llm_inputs.size()[:-1], dtype=torch.long).to(self.device)  # bsz x 1
        return llm_inputs, atts_llm
    ...
    def encode_audio(self, audio_paths):
        ...
        llm_inputs = self.llm_projection(audio_embeds).unsqueeze(1)  # bsz x 1 x llama_size
        atts_llm = torch.ones(llm_inputs.size()[:-1], dtype=torch.long).to(self.device)  # bsz x 1
        return llm_inputs, atts_llm
    ...
    def encode_image(self, image_paths):
        ...
        llm_inputs = self.llm_projection(image_embeds).unsqueeze(1)  # bsz x 1 x llama_size
        atts_llm = torch.ones(llm_inputs.size()[:-1], dtype=torch.long).to(self.device)  # bsz x 1
        return llm_inputs, atts_llm
    ...
    def prompt_wrap(self, img_embeds, input_ids, target_ids, attention_mask):
        ...
        bos = torch.ones([batch_size, 1], dtype=input_ids.dtype,
                         device=input_ids.device) * self.llm_tokenizer.bos_token_id  # bsz x 1
        if self.config['freeze_lm']:
            p_after_embeds = self.llm.model.embed_tokens(input_ids).expand(batch_size, -1,
                                                                                   -1)  # bsz x s2 x embed_dim
            bos_embeds = self.llm.model.embed_tokens(bos)  # bsz x 1 x embed_dim
        else:
            p_after_embeds = self.llm.model.model.embed_tokens(input_ids).expand(batch_size, -1,
                                                                                         -1)  # bsz x s2 x embed_dim
            bos_embeds = self.llm.model.model.embed_tokens(bos)  # bsz x 1 x embed_dim
        if img_embeds is not None:
            p_before = '### Human: <Img>'
            p_before_tokens = self.llm_tokenizer(p_before, return_tensors="pt", add_special_tokens=False).to(
                self.device)
            # peft model need deeper call
            if self.config['freeze_lm']:
                p_before_embeds = self.llm.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1,
                                                                                                        -1)  # bsz x s1 x embed_dim
            else:
                p_before_embeds = self.llm.model.model.embed_tokens(p_before_tokens.input_ids).expand(
    ...
    def _train_with_mode(self, texts, img_embeds=None, modality='text', num_gen_tokens='8',
        ...
        """
        if stage == 2:
            input_ids, target_ids, attention_mask = process_batch_stage_2(self.llm_tokenizer, texts,
                                                                          self.max_length,
                                                                          num_gen_tokens,
                                                                          modality
                                                                          )
        elif stage == 3:
            input_ids, target_ids, attention_mask = process_batch_stage_3(self.llm_tokenizer, texts, self.max_length,
                                                                          self.config['num_gen_img_tokens'],
                                                                          self.config['num_gen_video_tokens'],
                                                                          self.config['num_gen_audio_tokens']
                                                                          )
        else:
    ...
    def _enc_align_training_stage_1(self, inputs):
        ...
        if dataset_type == 'ImageToText':
            ...
            raise NotImplementedError
        input_ids, target_ids, attention_mask = process_batch_stage_1(self.llm_tokenizer,
                                                                      inputs['output_texts'],
                                                                      self.max_length,
                                                                      self.config['prompt'])
        # print(input_ids)
    ...
    def _dec_align_training_stage_2(self, inputs):
        ...
        if dataset_type == 'TextToImage':
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            modality='image',
                                                            num_gen_tokens=self.config['num_gen_img_tokens'],
                                                            text_hidden_fcs=self.gen_text_hidden_fcs,
                                                            gen_token_idx=self.config['gen_img_token_idx'],
                                                            text_emb_layers=self.config['text_emb_to_img_layers'],
                                                            text_prompt_embeddins=inputs['caption_embs'],
                                                            stage=self.stage)
        elif dataset_type == 'TextToVideo':
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            modality='video',
                                                            num_gen_tokens=self.config['num_gen_video_tokens'],
                                                            text_hidden_fcs=self.gen_text_hidden_fcs_video,
                                                            gen_token_idx=self.config['gen_video_token_idx'],
                                                            text_emb_layers=self.config['text_emb_to_video_layers'],
                                                            text_prompt_embeddins=inputs['caption_embs'],
                                                            stage=self.stage)
        elif dataset_type == 'TextToAudio':
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            modality='audio',
                                                            num_gen_tokens=self.config['num_gen_audio_tokens'],
                                                            text_hidden_fcs=self.gen_text_hidden_fcs_audio,
                                                            gen_token_idx=self.config['gen_audio_token_idx'],
                                                            text_emb_layers=self.config['text_emb_to_audio_layers'],
                                                            text_prompt_embeddins=inputs['caption_embs'],
                                                            stage=self.stage)
        else:
            raise NotImplementedError
    ...
    def _instruction_tuning_stage_3(self, inputs):
        ...
        if dataset_type == 'TextToImage':
            loss, gen_acc, mse_loss = self._train_with_mode(inputs['output_texts'], None, 'image',
                                                            ...
                                                            self.config['gen_img_token_idx'],
                                                            self.config['text_emb_to_img_layers'],
            ...
            loss, gen_acc, mse_loss = self._train_with_mode(inputs['output_texts'], None, 'video',
                                                            ...
                                                            self.config['gen_video_token_idx'],
                                                            self.config['text_emb_to_video_layers'],
            ...
            loss, gen_acc, mse_loss = self._train_with_mode(inputs['output_texts'], None, 'audio',
                                                            ...
                                                            self.config['gen_audio_token_idx'],
                                                            self.config['text_emb_to_audio_layers'],
                                                            inputs['caption_embs'], stage=self.stage)
    ...
    def _prepare_image_embed(self, text, batch_size):
        ...
        features = []
        p_before_token = self.llm_tokenizer('<Img>', add_special_tokens=False, return_tensors='pt').to(self.device)
        p_after_token = self.llm_tokenizer('</Img>', add_special_tokens=False, return_tensors='pt').to(self.device)
        if self.config['freeze_lm']:
            p_before_embeds = self.llm.model.embed_tokens(p_before_token.input_ids).expand(batch_size, -1,
                                                                                                   -1)  # bsz x s1 x embed_dim
            p_after_embeds = self.llm.model.embed_tokens(p_after_token.input_ids).expand(batch_size, -1,
                                                                                                 -1)  # bsz x s2 x embed_dim
        else:
            p_before_embeds = self.llm.model.model.embed_tokens(p_before_token.input_ids).expand(batch_size, -1,
                                                                                                         -1)  # bsz x s1 x embed_dim
            p_after_embeds = self.llm.model.model.embed_tokens(p_after_token.input_ids).expand(batch_size, -1,
                                                                                                       -1)  # bsz x s2 x embed_dim
        for m in matches:
            print('image path: ', m)
    ...
    def prepare_generation_embedding(self, inputs):
        ...
        for st in split_text:
            if st.startswith('Image>'):
                ...
                text_tokens = self.llm_tokenizer(st, add_special_tokens=False, return_tensors='pt').to(self.device)
                bos = torch.ones([batch_size, 1],
                                 dtype=text_tokens.input_ids.dtype,
                                 device=text_tokens.input_ids.device) * self.llm_tokenizer.bos_token_id  # bsz x 1
                if self.config['freeze_lm']:
                    text_embeds = self.llm.model.embed_tokens(text_tokens.input_ids).expand(batch_size, -1, -1)
                    bos_embeds = self.llm.model.embed_tokens(bos)  # bsz x 1 x embed_dim
                else:
                    text_embeds = self.llm.model.model.embed_tokens(text_tokens.input_ids).expand(batch_size,
                                                                                                          -1, -1)
                    bos_embeds = self.llm.model.model.embed_tokens(bos)  # bsz x 1 x embed_dim
    ...
    def generate_tokens_embeddings(self, inputs, input_embeds, temperature: float = 0.0, top_p: float = 1.0):
        ...
        for _hidden_states in outputs.hidden_states[1:]:
            for idx in self.config['text_emb_to_img_layers']:
                output_embeddings.append(_hidden_states[idx])
            for idx in self.config['text_emb_to_video_layers']:
                video_output_embedding.append(_hidden_states[idx])
            for idx in self.config['text_emb_to_audio_layers']:
    ...
    def generate_images(self, generated_ids, embeddings, all_gen_idx, generation_model=None,
        ...
        for gen_idx in all_gen_idx:
            assert generated_ids[0,
                   gen_idx:gen_idx + self.config['num_gen_img_tokens']].cpu().detach().numpy().tolist() == self.config[
                       'gen_img_token_idx'], (
                generated_ids[0, gen_idx:gen_idx + self.config['num_gen_img_tokens']], self.config['gen_img_token_idx'])
            raw_emb = embeddings[:, gen_idx - 1:gen_idx - 1 + self.config['num_gen_img_tokens'], :]  # (1, 8, 4096)

            # Produce generation embedding.
            gen_prefix = ' '.join([f'[IMG{i}]' for i in range(self.config['num_gen_img_tokens'])])
            gen_prefx_ids = self.llm_tokenizer(gen_prefix, add_special_tokens=False,
            ...
            caption = \
                self.llm_tokenizer.batch_decode(generated_ids[:, last_ret_idx:gen_idx], skip_special_tokens=True)[
                    0]
    ...
    def generate_videos(self, generated_ids, embeddings, all_gen_idx, generation_model=None,
        ...
        for gen_idx in all_gen_idx:
            assert generated_ids[0,
                   gen_idx:gen_idx + self.config['num_gen_video_tokens']].cpu().detach().numpy().tolist() == \
                   self.config[
                       'gen_video_token_idx'], (
                generated_ids[0, gen_idx:gen_idx + self.config['num_gen_video_tokens']],
                self.config['gen_video_token_idx'])
            raw_emb = embeddings[:, gen_idx - 1:gen_idx - 1 + self.config['num_gen_video_tokens'], :]  # (1, 8, 4096)
            # print(f'gen_idx: {gen_idx}')
            # print('4', raw_emb.size())
            # assert len(self.config['text_emb_to_video_layers']) == 1

            # Produce generation embedding.
            gen_prefix = ' '.join([f'[VID{i}]' for i in range(self.config['num_gen_video_tokens'])])
            gen_prefx_ids = self.llm_tokenizer(gen_prefix, add_special_tokens=False,
                                                 return_tensors="pt").input_ids.to(self.device)
            gen_prefix_embs = self.input_embeddings(gen_prefx_ids)  # (1, T_I_V_A.txt, D)
    ...
    def generate_audios(self, generated_ids, embeddings, all_gen_idx, generation_model=None,
        ...
        for gen_idx in all_gen_idx:
            assert generated_ids[0,
                   gen_idx:gen_idx + self.config['num_gen_audio_tokens']].cpu().detach().numpy().tolist() == \
                   self.config[
                       'gen_audio_token_idx'], (
                generated_ids[0, gen_idx:gen_idx + self.config['num_gen_audio_tokens']],
                self.config['gen_audio_token_idx'])
            raw_emb = embeddings[:, gen_idx - 1:gen_idx - 1 + self.config['num_gen_audio_tokens'], :]  # (1, 8, 4096)
            # print(f'gen_idx: {gen_idx}')
            # print('raw_emb 4', raw_emb.size())
            # assert len(self.config['text_emb_to_video_layers']) == 1

            # Produce generation embedding.
            gen_prefix = ' '.join([f'[AUD{i}]' for i in range(self.config['num_gen_audio_tokens'])])
            gen_prefx_ids = self.llm_tokenizer(gen_prefix, add_special_tokens=False,
                                                 return_tensors="pt").input_ids.to(self.device)
            gen_prefix_embs = self.input_embeddings(gen_prefx_ids)  # (1, T_I_V_A.txt, D)
    ...
    def generate(self, inputs):
        ...
        all_gen_img_idx = [i for i, x in enumerate(generated_ids[0, :] == self.config['gen_img_token_idx'][0]) if x][
                          :inputs['max_num_imgs']]
        print('all_gen_img_idx: ', all_gen_img_idx)

        # Find up to max_num_rest [VID] tokens, and their corresponding scores.
        all_gen_vid_idx = [i for i, x in enumerate(generated_ids[0, :] == self.config['gen_video_token_idx'][0]) if x][
                          :inputs['max_num_vids']]
        print('all_gen_vid_idx: ', all_gen_vid_idx)

        # Find up to max_num_rest [AUD] tokens, and their corresponding scores.
        all_gen_aud_idx = [i for i, x in enumerate(generated_ids[0, :] == self.config['gen_audio_token_idx'][0]) if x][
                          :inputs['max_num_auds']]
        ...
        if len(all_gen_img_idx) == 0 and len(all_gen_vid_idx) == 0 and len(all_gen_aud_idx) == 0:
            ...
            caption = self.llm_tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0]
